/*
   a-simple-web-server'ın multi-thread hale getirilmiş versiyonu

   Dezavantaj;

   Sistem thread'ler arası geçip yapar ve bunun bir maliyeti olur. context switching.
   Ayrıca thread'ler ağır(heavyweight) nesnelerdir. Günümüz donanımları için bu bir problem olmasa
   da, I/O bloklamaları halen geçerlidir. Bunun için a-non-blocking-server alternatifi vardır.
...
fn main() {
    ...
    loop {
        ...
        // her request için ayrı bir thread açılıyor
